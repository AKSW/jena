    ...
    implements JenaSubsystemLifecycle
{
    ...
    private static QueryIterator execute(Op op, DatasetGraph dataset, Binding binding, Context cxt) {
        ...
        if (ddg != null) {
            ...
            Query baseQuery = OpAsQuery.asQuery(op);
            ...
            ExecutionContext execCxt = qIter instanceof QueryIter ? ((QueryIter)qIter).getExecContext() : null;
            qIter = new QueryIteratorMapped(qIter, normedToScoped);
            result = new QueryIterCommonParent(qIter, binding, execCxt);
        }

        if (result == null) {
            QueryEngineFactory qef = QueryEngineRegistry.findFactory(op, dataset, cxt);
            Plan plan = qef.create(op, dataset, binding, cxt);
            result = plan.iterator();
        }
        return result;
    }
