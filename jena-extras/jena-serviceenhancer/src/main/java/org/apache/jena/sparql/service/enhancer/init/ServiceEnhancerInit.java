    ...
    implements JenaSubsystemLifecycle
{
        ...
        ChainingServiceExecutor selfExec = (opExec, opOrig, binding, execCxt, chain) -> {
            ...
            if (ServiceEnhancerConstants.SELF.equals(target.getService())) {
                ...
                if (useQc) {
                    ...
                    Plan plan = createPlan(op, dataset, binding, cxtCopy);
                    r = plan.iterator();
                }
            } else {
    ...
    private static Plan createPlan(Op op, DatasetGraph dataset, Binding binding, Context cxt) {
        Plan plan = null;
        ...
        if (ddg != null) {
            ...
            plan = qef.create(effQuery, effDataset, binding, cxt);
        }

        if (plan == null) {
            QueryEngineFactory qef = QueryEngineRegistry.findFactory(op, dataset, cxt);
            plan = qef.create(op, dataset, binding, cxt);
        }
