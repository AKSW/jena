...
public class SpatialIndexComputeService extends BaseActionREST { //ActionREST {
    ...
    protected void doPost(HttpAction action) {
        ...
        try {
            ...
            Dataset ds = DatasetFactory.wrap(action.getActiveDSG());
            try {
                action.log.info(format("[%d] spatial index: computation started", action.id));
                if (inMemory) {
                    SpatialIndex.buildSpatialIndex(ds, true);
                } else {
                    SpatialIndex.buildSpatialIndex(ds, new File(spatialIndexFilePathStr), true);
                }
